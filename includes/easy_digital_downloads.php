...
function generate_xml(){
    ...
    foreach ($products as $product){
        ...
        $categories_output = '';
        $categories = get_the_terms( $product->ID, 'download_category' );
        if(!empty($categories)){
            $i = 0;
        ...
        $price = get_post_meta($product->ID,'edd_price',true);

        if(!empty($price)){
            $output .= '<price><![CDATA['.get_post_meta($product->ID,'edd_price',true).']]></price>';
        }
...
function get_category_breadcrumb($category){
    if($category->parent == 0){
        ...
        while($category->parent != 0){
            $category = get_term($category->parent, 'download_category', 'OBJECT');
